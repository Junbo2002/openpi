...
class AgiInputs(transforms.DataTransformFn):
    ...
    action_mask = [False] * 7 + [True] * 7 + [False, True] + [False] * 12
    ...
    def __call__(self, data: dict) -> dict:
        ...
        # left_wrist_image = _parse_image(data["observation.images.hand_left"])
        # right_wrist_image = _parse_image(data["observation.images.hand_right"])
        
        base_image = _parse_image(data["images"]["head"])
        left_wrist_image = _parse_image(data["images"]["hand_left"])
        right_wrist_image = _parse_image(data["images"]["hand_right"])
        # from PIL import Image
        # Image.fromarray(base_image).save("base_image.png")
        # Image.fromarray(left_wrist_image).save("left_wrist_image.png")
        # Image.fromarray(right_wrist_image).save("right_wrist_image.png")
        ...
        if "actions" in data:
            ...
            inputs["actions"] = actions[:, self.action_mask]
